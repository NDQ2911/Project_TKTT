...
func main() {
	...
	// Load custom index settings if available
	var customSettings []byte
	settingsPath := "config/index_settings.json"
	if data, err := os.ReadFile(settingsPath); err == nil {
		customSettings = data
		log.Printf("Loaded custom index settings from %s (%d bytes)", settingsPath, len(data))
	} else {
		log.Printf("Using default index settings (no %s found)", settingsPath)
	}

	// Ensure index exists with proper mapping
	if err := esIndexer.EnsureIndexWithSettings(ctx, customSettings); err != nil {
