...
func (n *Normalizer) Normalize(raw *domain.RawJob) (*domain.Job, error) {
	...
	// Clean short text fields, keep HTML for long content
	job.Title = cleanTitle(job.Title)
	job.Company = cleanTitle(job.Company)
	job.Location = cleanTitle(job.Location)
	// Description, Requirements, Benefits - keep HTML as-is for frontend rendering
...
// cleanTitle cleans short text fields (title, company, location)
...
func cleanTitle(s string) string {
	...
	// Double decode HTML entities (some sources encode twice)
...
// cleanText cleans long-form text fields (description, requirements, benefits)
// - Double decode HTML entities
// - Remove/convert HTML tags
// - Normalize bullet points (•, -, *, ▪) to consistent format
// - Fix line breaks and whitespace
// - Make text more readable for search and display
func cleanText(s string) string {
	...
	// Detect section headers (all caps Vietnamese text like "KỸ SƯ THIẾT KẾ...")
	// Add newline before them to separate sections
	sectionHeaderRe := regexp.MustCompile(`([A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰỲÝỶỸỴ][A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰỲÝỶỸỴ\s]{10,})`)
	s = sectionHeaderRe.ReplaceAllString(s, "\n\n$1\n")

	// Normalize different bullet point characters to consistent format
	// Vietnamese commonly uses: •, -, *, ▪, ●, ○, ►, ✓
	bulletRe := regexp.MustCompile(`(?m)^[\s]*[•\-\*▪●○►✓➢➤✔]\s*`)
	s = bulletRe.ReplaceAllString(s, "\n• ")

	// Also handle inline bullet points (within text, not at line start)
	inlineBulletRe := regexp.MustCompile(`[•▪●○►✓➢➤✔]\s*`)
	s = inlineBulletRe.ReplaceAllString(s, "\n• ")

	// Handle numbered lists (1., 2., etc.)
	numberedRe := regexp.MustCompile(`(?m)^\s*(\d+)[\.\)]\s*`)
	s = numberedRe.ReplaceAllString(s, "\n$1. ")

	// Split merged sentences: lowercase followed by uppercase letter
	// e.g. "công việc.Cập nhật" -> "công việc.\nCập nhật"
	sentenceSplitRe := regexp.MustCompile(`([.!?])([A-ZÀÁẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬĐÈÉẺẼẸÊẾỀỂỄỆÌÍỈĨỊÒÓỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÙÚỦŨỤƯỨỪỬỮỰỲÝỶỸỴ][a-zàáảãạăắằẳẵặâấầẩẫậđèéẻẽẹêếềểễệìíỉĩịòóỏõọôốồổỗộơớờởỡợùúủũụưứừửữựỳýỷỹỵ])`)
	s = sentenceSplitRe.ReplaceAllString(s, "$1\n$2")
