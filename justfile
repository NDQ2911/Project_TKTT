...
dev:
    Start-Process just -ArgumentList "backend"
    Start-Process just -ArgumentList "frontend"
...
test-http:
    curl.exe -X POST http://localhost:3000/search/http -H "Content-Type: application/json" -d '{"q":"IT"}'

test-sdk:
    curl.exe -X POST http://localhost:3000/search/sdk -H "Content-Type: application/json" -d '{"q":"IT"}'
